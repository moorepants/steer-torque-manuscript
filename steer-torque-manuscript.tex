...
lighter-weight ones, but it is well known that forces applied to the handlebar
give much more control authority than other means regardless of the vehicle's
inertia \cite{Sharp2007,Sharp2008a}.

From a single track vehicle modeling perspective, it is generally easier to
model the human/machine interface forces as a single torque about the steer
axis which acts between the front and rear frames. The rider can be assumed to
be part of the rear frame, regardless of whether rider is assumed to be rigid
or not.
...
errors. We then give the computations needed to correctly compensate for
...
maneuvers are in the 4 to 14 Nm range. Not long after this Kondo \cite{Kondo1955}
...
torque in the United States. He attached an additional handle bar above the regular
...
\cite{James2002} developed a secondary handlebar with an integrated load cell
to measure steer torques in an off-road motorcycle for the purposes of
identifying the a model that relates steer torque inputs to vehicle kinematical
ouputs.
...
the front frame above or below the sensor, except Iuchi \cite{Iuchi2006}.
Evertse \cite{Evertse2010} may have the only design which mitigates this
inertial compensation issue completely.
...
The various designs can be classified in into

\begin{description}
  \item[Mechanical] The rider applies a torque to the causes a needle to indicate
  \item[Simple Strain Gages] Strain gages are applied to the vehicles stock
    handlebars, typically near or on the steer column
  \item[Floating handlebar with inline torque transducer] The handlebars are
    separated from the fork, each mounted in bearings, and the two are
    connected by and inline force transducer.
  \item[Floating handlebar with linear transducer] The handlebars are separated
    from the fork, each mounted in bearings, and the two are connected through
    a linear force transducer.
  \item[Floating handlebar with cantilever transducer] The handlebars are separated
    from the fork, each mounted in bearings, and the two are connected through
    a cantilever beam force transducer.
  \item[Load cells in the grips]
  \item[Estimated from motor power]
\end{description}
...
Our design is based around a Futek (Irvine, CA, USA) 150 in-lb ($\pm 17$ Nm)
TFF350 torque sensor to ensure high accuracy for the low torques used in normal
...
Figure~\ref{fig:steer-torque-design}.
